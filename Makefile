...
build: $(MAKO_FILES:.mako=) \
	...
	docker/admin-dev/Dockerfile \
	docker/admin-dev/npm-packages
...
flake8:
	...
	git grep --files-with-match '/usr/bin/env python' | grep -v Makefile | xargs flake8 --max-line-length=110 \
		--ignore=E712 \
		--max-line-length=110 \
		--copyright-check \
		--copyright-min-file-size=1 \
...
docker/admin-dev/npm-packages: admin/package.json $(BUILD_DIR)/requirements.timestamp geoportal/c2cgeoportal_geoportal/scripts/import_ngeo_apps.py
	$(PRERULE_CMD)
	$(BUILD_DIR)/venv/bin/import-ngeo-apps --package _ $< $@
